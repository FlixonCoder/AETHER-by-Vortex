...
(function () {
  ...
  let _geoSmall = null, _geoBig = null;
  const _matCache = new Map();

  function materialFor(color) {
    if (!_matCache.has(color)) {
      _matCache.set(color, new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.95 }));
    }
  ...
  function satMesh(d) {
    if (!_geoSmall) {
      _geoSmall = new THREE.SphereGeometry(0.7, 8, 6);
      _geoBig = new THREE.SphereGeometry(1.9, 12, 10);
    }
    return new THREE.Mesh(d.isLyra ? _geoBig : _geoSmall, materialFor(d.color));
  }
  ...
  async function boot() {
    ...
    globe = Globe()(host)
      ...
      .onGlobeClick(() => { selected = null; updateInfoPanel(null); refresh(); });
