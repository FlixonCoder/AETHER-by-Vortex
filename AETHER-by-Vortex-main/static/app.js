...
const icoUse = (id) => `<svg class="ico"><use href="#${id}"/></svg>`;
const SUBSYSTEMS = {
  EPS: { name: 'Power System', ico: icoUse('i-battery') },
  THERMAL: { name: 'Thermal System', ico: icoUse('i-thermo') },
  COMMS: { name: 'Comm System', ico: icoUse('i-antenna') },
  ADCS: { name: 'Attitude Control', ico: icoUse('i-compass') },
  OBC: { name: 'On-Board Computer', ico: icoUse('i-cpu') },
  PAYLOAD: { name: 'Payload System', ico: icoUse('i-camera') },
  PROPULSION: { name: 'Propulsion', ico: icoUse('i-thruster') },
};
...
function renderLogs() {
  // The Overview log card was dropped in the redesign — logs now live only
  // in the modal, reachable from the topbar. Guarded rather than deleted so
  // this keeps working if that card ever comes back.
  const ovBox = $('logStreamOv');
  if (ovBox) {
    const fl = filteredLogs($('logSearchOv') ? $('logSearchOv').value : '', $('logFilterOv') ? $('logFilterOv').value : '');
...
/* ---------------- Overview: minimise the Satellite Status panel ---------------- */
...
function handleMessage(msg) {
  ...
  switch (msg.type) {
    ...
    case 'baseline_check_complete': onBaselineCheck(d, msg.timestamp); break;
    case 'trust_score_update': onTrustScoreUpdate(d, msg.timestamp); break;
...
function renderOverview() {
  ...
  $('ovEclipse').textContent = latestOrbital.in_eclipse ? 'ECLIPSE' : 'SUNLIT';
  ...
  // Quick Health Check (the ring + subsystem list) was removed from Overview
  // in the redesign — the full version lives on the merged Telemetry & Health
  // page. `overall` is still computed because satTag below needs it.
  if ($('ringPct')) {
...
/* ---------------- Workflow ---------------- */
const WF_STAGES = [
  { name: 'Telemetry Ingest', ico: icoUse('i-stream') },
  { name: 'Watcher Detection', ico: icoUse('i-eye') },
  { name: 'Criticality Scoring', ico: icoUse('i-scale') },
  { name: 'Identifier Agent', ico: icoUse('i-search') },
  { name: 'Fix Finder Agent', ico: icoUse('i-puzzle') },
  { name: 'Digital Twin Sim', ico: icoUse('i-flask') },
  { name: 'Baseline Check', ico: icoUse('i-checklist') },
  { name: 'Safety Gate', ico: icoUse('i-shield') },
  { name: 'Execute & Verify', ico: icoUse('i-launch') },
];
...
function onAgentActivity(d, ts) {
  ...
  if (d.agent === 'BASELINE_CHECK') { wfSet(5, 'done', null, ts); wfSet(6, 'active', 'Checking predicted state against full telemetry baseline…'); }
  if (d.agent === 'SAFETY_GATE') { wfSet(6, 'done', null, ts); wfSet(7, 'active', 'Safety Gate verifying deterministic policy…'); }
  if (d.agent === 'EXECUTOR') { wfSet(7, 'done', null, ts); wfSet(8, 'active', 'Executing authorized commands on spacecraft…'); }
  if (d.agent === 'POST_MONITOR') wfSet(8, 'active', 'Verifying post-execution telemetry recovery…');
}

function onBaselineCheck(d, ts) {
  const b = d.baseline_check || {};
  const passOk = b.checked_count != null ? `${b.checked_count - b.violation_count}/${b.checked_count} parameters` : 'baseline';
  addLog(b.passed ? 'OK' : 'WARN', `Baseline check: ${passOk} — ${b.reasoning || ''}`, ts);
  wfSet(6, 'done', b.passed ? 'All parameters within safe operating band' : `${b.violation_count} parameter(s) out of band`, ts);
  wfSet(7, 'active', 'Deterministic Safety Gate evaluating policy…');
}

function onTrustScoreUpdate(d, ts) {
  if (d.overall_trust_score == null) return;
  addLog('INFO', `Incident ${d.incident_id} solution trust score: ${d.overall_trust_score}/100 (avg of ${d.stage_trail?.length || 0} agent stage(s))`, ts);
}
...
function onSimulationComplete(d, ts) {
  ...
  wfSet(6, 'active', 'Checking predicted state against full telemetry baseline…');
}
...
function onValidationDecision(d, ts) {
  ...
  wfSet(7, 'done', `Safety gate cleared: ${val.decision}`, ts);
}

function onCommandExecuted(d, ts) {
  const ex = d.execution || {};
  addLog('OK', `Command dispatch complete: ${ex.status}`, ts);
  wfSet(8, 'active', 'Commands dispatched — awaiting telemetry stabilization…');
}
...
function onPostMonitorResult(d, ts) {
  ...
  if (res.recovered) {
    ...
    wfSet(8, 'done', 'Telemetry restored to nominal limits', ts);
  } else {
...
/* ---------------- Logs UI ---------------- */
// logSearchOv/logFilterOv/logsViewAll lived on the Overview log card, which
// the redesign removed (Overview is now hero + two HUD panels only). The
// modal's only other opener is the topbar Logs button (#logsOpenBtn) below.
...
$('logsOpenBtn')?.addEventListener('click', () => { $('logsModal').classList.add('open'); renderModalLogs(); });
